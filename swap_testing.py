...
from run_experiment import (compute_distinguisher_score,
                            ...
                            select_device, train_single_model)
from components.unlearning_methods import *
from distinguishers.swap_test import run_swap_test
...
def run_unlearning(
    ...
    **kwargs
    ...
    torch.save(rt_model.state_dict(), str(unlearned_model_dir / f'{unlearning_method}_rt_state_dict.pt'))
    
    return unlearned_model, rt_model
...
def main():
    ...
    score_dicts = {d: {k: [] for k in ('forget_set', 'run_number', 'model', 'score')} for d in ('clean_accuracy', 'poison_accuracy', 'poison_tpr', 'swap_test_score', *distinguisher_params.keys())}
    for forget_set_ind, poison_rate in enumerate(dataset_params['poison_rate']):
        ...
        for run_num in range(config_dict['runs_per_forget_set']):
            ...
            # Now subsample the test set so it's the same size as the forget set
            poison_test_ds = Subset(poison_test_ds, indices=list(range(len(forget_ds))))
            clean_test_ds = Subset(clean_test_ds, indices=list(range(len(forget_ds))))
            ...
            for unlearning_method_key, unlearning_params in unlearning_methods.items():
                ...
                rt_model_ckpt = unlearned_model_dir / f'{unlearning_method}_rt_state_dict.pt'
                print(f'Running {unlearning_method_key}')
                if not args.redo_unlearning and args.use_existing_models and unlearned_model_ckpt.exists() and rt_model_ckpt.exists():
                    ...
                    if unlearning_method == 'certified_removal':
                        rt_model.fc = nn.Linear(rt_model.fc.in_features, num_classes, bias=False) # re-initialize the final layer but remove the bias term
                    rt_model.load_state_dict(torch.load(str(rt_model_ckpt), weights_only=True))
                    rt_model.to(device)
                else:
                    unlearned_model, rt_model = run_unlearning(
                ...
                models_to_score[unlearning_method_key] = (unlearned_model, rt_model)
            ...
            # Add in clean and poisoned performance and swap scores
            for model_name, model in tqdm(models_to_score.items(), desc=f'Scoring models for {distinguisher}'):
                # SWAP testing
                if isinstance(model, tuple):
                    model, rt_model = model
                    swap_quality_score = run_swap_test(
                        rf_model=model,
                        rt_model=rt_model,
                        forget_ds=forget_ds,
                        test_ds=poison_test_ds,
                    ...
                    score_dicts['swap_test_score']['forget_set'].append(forget_set_name[len("forget_"):])
                    score_dicts['swap_test_score']['run_number'].append(run_num)
                    score_dicts['swap_test_score']['model'].append(model_name)
                    score_dicts['swap_test_score']['score'].append(swap_quality_score)
